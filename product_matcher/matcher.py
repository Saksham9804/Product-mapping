...
def retrieve_candidates(source: PlatformDataset, target: PlatformDataset, config: MatchConfig) -> pd.DataFrame:
    ...
    rows = []
    active_groups = sorted(set(source_df["group_key"]).intersection(set(target_df["group_key"])))

    for group_key in active_groups:
        left = source_df[source_df["group_key"].eq(group_key)].reset_index(drop=True)
        right = target_df[target_df["group_key"].eq(group_key)].reset_index(drop=True)
